...
vector<vector<contour>> add_contours;
vector<contour> avg_contours;

vector<vector<float>> add_images;
vector<float> avg_image;
...
vector<float> get_average(vector<vector<float>>& images, size_t target_res)
{
	vector<float> s(target_res * target_res, 0);
